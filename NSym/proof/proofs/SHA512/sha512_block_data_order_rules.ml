...
let commutativity_and_associativity_of_bvadd_1 =
  ...
  let sigma_big_1 = (get_air_fn "spec.SHA512rec.air_S1") in
  let ch    = (get_air_fn "spec.SHA512rec.air_Ch") in
  let x0 = (sb 64 "x0") in
  let x1 = (sb 64 "x1") in
  let x2 = (sb 64 "x2") in
  let x3 = (sb 64 "x3") in
...
let commutativity_and_associativity_of_bvadd_2 =
  ...
  let sigma_big_0 = (get_air_fn "spec.SHA512rec.air_S0") in
  let maj   = (get_air_fn "spec.SHA512rec.air_Maj") in
  let x0 = (sb 64 "x0") in
  let x1 = (sb 64 "x1") in
  let x2 = (sb 64 "x2") in
  let x3 = (sb 64 "x3") in
...
let commutativity_and_associativity_of_bvadd_3 =
  ...
  let sigma_1 = (get_air_fn "spec.SHA512rec.air_s1") in
  let sigma_0 = (get_air_fn "spec.SHA512rec.air_s0") in
  let x0 = (sb 64 "x0") in
  let x1 = (sb 64 "x1") in
  let x2 = (sb 64 "x2") in
  let x3 = (sb 64 "x3") in
  ...
  let sigma0_equiv_rule = 
    ...
    let spec = get_air_fn "spec.SHA512rec.air_s0" in
    let a = (sb 64 "a") in
  ...
  let sigma1_equiv_rule = 
    ...
    let spec = get_air_fn "spec.SHA512rec.air_s1" in
    let a = (sb 64 "a") in
  ...
  let maj_equiv_rule = 
    ...
    let spec = get_air_fn "spec.SHA512rec.air_Maj" in
    let a = (sb 64 "a") in
    let b = (sb 64 "b") in
    let c = (sb 64 "c") in
  ...
  let ch_equiv_rule = 
    ...
    let spec = get_air_fn "spec.SHA512rec.air_Ch" in
    let a = (sb 64 "a") in
    let b = (sb 64 "b") in
    let c = (sb 64 "c") in
    let lhs = (apply impl [a;b;c]) in
...
let crock1 =
  ...
  let eor_i = (get_air_fn "arm.inst_dpr_logical_shifted_reg.eor64") in
  let ror_i = (get_air_fn "arm.inst_dpi_extract.ror64") in
  let e = (s_cb 64 "0x510e527fade682d1") in
  let e_rot = (s_cb 64 "0xcd05a2a21ca4ff5b") in
  let s0 = 14 in
  let s1 = 18 in
...
let crock2 =
  ...
  let eor_i = (get_air_fn "arm.inst_dpr_logical_shifted_reg.eor64") in
  let ror_i = (get_air_fn "arm.inst_dpi_extract.ror64") in
  let a = (s_cb 64 "0x6a09e667f3bcc908") in
  let a_rot = (s_cb 64 "0x43504f333f9de648") in
  let s0 = 28 in
  let s1 = 34 in
  let big_t0 = (apply ror_i [(cb 32 s0); a; a]) in
